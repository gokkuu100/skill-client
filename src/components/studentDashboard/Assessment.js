  ...
  const handleRowClick = (assessment) => {
    if (assessment.isCompleted) {
      const shouldRedo = window.confirm(
        'This assessment was already done. Do you want to redo your assessment?'
      );
      if (shouldRedo) {
  ...
  return (
    <div className="flex flex-col w-full">
      <div className="w-full p-4 bg-gray-800 flex items-center">
        ...
        <h1 className="text-white ml-[55rem]">ASSESSMENTS</h1>
      </div>
      <h3 className='Notice text-center text-xl font-semibold text-gray-800 mt-4 mb-2'>Select an assessment to attempt if not yet done</h3>
              ...
              >
                ...
                <td
                  className={`p-4 text-center border ${
                    assessment.isCompleted ? 'text-green-500' : 'text-red-500'
                  }`}
                >
